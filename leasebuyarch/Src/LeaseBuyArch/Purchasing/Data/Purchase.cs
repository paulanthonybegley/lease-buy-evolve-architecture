namespace EvolutionaryArchitecture.LeaseBuyArch.Purchasing.Data;
...
internal sealed class Purchase
{
    ...
    internal static Purchase Offer(Guid customerId, Guid vehicleId, decimal vehicleMsrp,
    ...
    private static decimal CalculateMonthlyPayment(decimal msrp, decimal downPayment, decimal apr, int termMonths)
    {
        ...
        var payment = loanAmount * (monthlyRate * (decimal)Math.Pow(1 + (double)monthlyRate, termMonths)) /
