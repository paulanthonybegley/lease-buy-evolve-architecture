...
internal static class EventBusModule
{
    ...
    internal static IServiceCollection AddEventBus(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddMassTransit(configurator =>
        {
            ...
            configurator.UsingRabbitMq((context, factoryConfigurator) =>
            {
                ...
                if (!externalEventBusConfigured) return;
    ...
    private static void ConfigureRabbitMq(IOptions<EventBusOptions> options, IRabbitMqBusFactoryConfigurator factoryConfigurator)
    {
        ...
        if (eventBusOptions.Uri is null || eventBusOptions.Username is null || eventBusOptions.Password is null) return;

        factoryConfigurator.Host(eventBusOptions.Uri, rabbitMqHostConfigurator =>
        {
            rabbitMqHostConfigurator.Username(eventBusOptions.Username);
            rabbitMqHostConfigurator.Password(eventBusOptions.Password);
        });
    }
